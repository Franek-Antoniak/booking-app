...
class ScreeningServiceImpl extends ScreeningService {
	...
	public Optional<ScreeningBookingInfoDTO> searchAvailableSeats(UUID screeningId) {
		...
		List<Seat> seats = screeningRoomRepository.findById(screeningEntity.getScreeningRoom().getId())
				.orElseThrow().getSeats();
		List<Booking> bookings = bookingRepository.findAllByIdIn(screeningEntity.getBookings().stream()
				.map(Booking::getId)
